...
# Alpine Linux base image modified for Docker
```shell
docker pull juliolustosa/alpine:3.6
```
...
The image has an `deploy` user with UID 9999 and home directory `/home/deploy`. Even though Docker itself provides some isolation from the host OS, running applications without root privileges is good security practice.
